...
@Component({
  ...
  template: `
    <div class="row">
      <div
        *ngFor="let productItem of productItems; let i = index"
      ...
      >
        <div *ngIf="i < setNumber">
          <p class="today-deal-timer" [class.active]="activeTimer">
            {{ hours < 10 ? '0' + hours : hours }}:{{
              minutes < 10 ? '0' + minutes : minutes
            }}:{{ seconds < 10 ? '0' + seconds : seconds }}남음
          </p>
          <div *ngIf="activeRank" class="itemRanking">
            <app-rank-crown [i]="i"></app-rank-crown>{{ i + 1 }}
          </div>
          <article class="store-index-today-deal-item">
            <a
              class="store-index-today-deal-item__overlay"
              routerLink="/store/{{ productItem.id }}"
            ></a>
            <div class="store-index-today-deal-item__image">
              <div class="production-item-image">
                <img
                  class="image"
                  [ngStyle]="hovered === i ? style : ''"
                  src="{{ productItem.thumnail_images[0].image }}"
                  alt
                />
              </div>
            </div>
            <div class="store-index-today-deal-item__content">
              <h1 class="store-index-today-deal-item__header">
                <span class="store-index-today-deal-item__header__brand">{{
                  productItem.brand_name
                }}</span>
                <span class="store-index-today-deal-item__header__name">{{
                  productItem.name
                }}</span>
              </h1>
              <span class="production-item-price">
                <span class="production-item-price__rate"
                  ...
                  }}<span class="percentage" *ngIf="productItem.discount_rate"
                    >{{ productItem.discount_rate }}%</span
                  >
                </span>
                <span class="production-item-price__price">{{
                  commonService.addComma(productItem.price)
                }}</span>
              </span>
